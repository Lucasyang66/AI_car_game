...
let step = new ActivationFunction(
  x => (x >= 0) ? 1 : 0,
  y => 0
);
...
class NeuralNetwork {
  ...
  */
  constructor(in_nodes, hid_nodes, out_nodes) {
    ...
    this.setActivationFunctionHidden();
    this.setActivationFunctionOutput();
  ...
  predict(input_array) {
    ...
    // activation function!
    hidden.map(this.activation_hidden.func);

    // Generating the output's output!
    let output = Matrix.multiply(this.weights_ho, hidden);
    output.add(this.bias_o);
    output.map(this.activation_output.func);
  ...
  setActivationFunctionOutput(func=step) {
    this.activation_output = func;
  }
  ...
  train(input_array, target_array) {
    ...
    outputs.map(this.activation_output.func);
